...
class ImageSlideshow(QWidget):
    def __init__(self, image_dir, masked_image_dir, train_dir, mask_capture_dir):
        ...
        self.cleanup_masked_done = False
        self.cleanup_mask_capture_done = False
        self.cleanup_train_done = False
    ...
    def start_game_timer(self, duration):
        ...
        self.game_timer.timeout.connect(lambda: self.display_message("TIME IS UP"))
        self.game_timer.start(duration)

    def show_masked_image(self):
        if not self.cleanup_masked_done:
            self.cleanup_dir(self.masked_image_dir)
            self.cleanup_masked_done = True
    ...
    def cleanup_dir(self, directory_to_cleanup):
        if os.path.exists(directory_to_cleanup):
            for f in os.listdir(directory_to_cleanup):
                file_path = os.path.join(directory_to_cleanup, f)
                try:
    ...
    def handle_button(self, result, correct_value):
        masked_image_path = self.get_last_masked_image()
        ...
        # List to store odd integers
        pattern_numbers = []
        # Loop through each string in the list
        for image in image_list:
            # Extract the digit from the string (assuming the digit is always at the end)
            digit_str = ''.join(filter(str.isdigit, image))
            # Convert the digit to an integer
            digit = int(digit_str)
            # Check if the digit is odd
            if digit % 2 != 0:
                pattern_numbers.append(digit)

        if result and visibility_value == constants['visibility_minimum'] and correct_value in pattern_numbers:
            pattern_numbers.remove(correct_value)
            if len(pattern_numbers) == 0:
                self.display_message("YOU WON")

        new_visibility_value = random.randint(2, 8) / 10
        ...
        if sender:
            ...
            if len(numbers_in_filename) >= 2:
                ...
                if button_number == correct_value:
                    ...
                    QTimer.singleShot(constants['feedback_message_duration'], lambda: self.handle_button(True, correct_value))
                else:
                    print("WRONG")
                    self.display_feedback(False, correct_value)
                    QTimer.singleShot(constants['feedback_message_duration'], lambda: self.handle_button(False, correct_value))
            else:
    ...
    def capture_and_save_image(self, filename, train_or_mask):
        ...
        if train_or_mask == 'train':
            if not self.cleanup_train_done:
                self.cleanup_dir(self.train_dir)
                self.cleanup_train_done = True

            # Save the image with to the trainimgs location
            filename_train = os.path.join(self.train_dir, filename)
            img.save(filename_train)
        else:
            if not self.cleanup_mask_capture_done:
                self.cleanup_dir(self.mask_capture_dir)
                self.cleanup_mask_capture_done = True
