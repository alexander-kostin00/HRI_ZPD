...
In this condition the robot gets presented images with the amount of the patterns covered increased or decreased (depending on performance) by a fixed percentage.
"""
...
project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../HRI_ZPD'))
...
class ImageSlideshow(QWidget):
    ...
    def start_game_timer(self, duration):
        """Start the game timer to end the game after the specified duration."""
        self.game_timer = QTimer(self)
        self.game_timer.setSingleShot(True)
        self.game_timer.timeout.connect(self.on_game_timeout)
        self.game_timer.start(duration)

    def on_game_timeout(self):
        """Handle actions when the game timer runs out."""
        self.closing = True
        # Save current iteration data
        self.summary_data["iterations"].append(self.current_iteration_data)

        # Display the "TIME IS UP" message
        QTimer.singleShot(3000, lambda: self.display_message("TIME IS UP")) # delay to wait for Correct/Incorrect to dissapear


        # After 2 seconds, display the completion message
        QTimer.singleShot(3500, self.display_completed_message)
    ...
    def show_masked_image(self):
        ...
        if not self.cleanup_done:
            ...
            apply_mask(constants['mask_visibility_initial'],
                       random.randint(constants['mask_pieces_min'], constants['mask_pieces_max']),
                       self.masked_image_dir)

        self.start_game_timer(constants['game_duration'])
        ...
        for i in range(button_count):
            ...
            button.setFont(QFont('Arial', 16))
            button.clicked.connect(self.check_button)
            buttons.append(button)  # Store buttons in a list for simulation
            button_layout.addWidget(button)
        ...
        # Simulate button press after displaying the image, comment out for human
        QTimer.singleShot(500, lambda: self.simulate_button_press(buttons))

    def simulate_button_press(self, buttons):
        # Simulate a button press based on robot pattern identified
        if buttons:
            print(self.pattern_identified)
            QTimer.singleShot(500, buttons[
                self.pattern_identified - 1].click)  # Simulate a click on the first button after 500 ms
    ...
    def after_capture_processing(self):
        ...
        print("the identified pattern is", self.pattern_identified)
    ...
    def handle_button(self, result):
        ...
        print("the visibility is", visibility_value)
        if result and visibility_value <= constants['visibility_minimum']:
            QTimer.singleShot(1000, lambda: self.display_message("YOU WON"))
            ...
            possible_values = [i / 100 for i in range(int(constants['visibility_minimum']*100),  int(constants["visibility_maximum"]*100) + 5, 5)]
            new_visibility_value = random.choice(possible_values)
