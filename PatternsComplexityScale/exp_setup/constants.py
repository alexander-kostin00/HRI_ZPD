"""
Author: Oleksandr Kostin
"""

constants = {
    'feedback_message_duration': 3000,       # milliseconds
    'pattern_image_display_duration': 700,  # milliseconds
    'number_image_display_duration': 200,   # milliseconds
    'mask_visibility_initial': 0.4,
    'mask_pieces_min': 1,
    'mask_pieces_max': 5,
    'decreasing_step': 2,    # value by which visibility percentage is divided
    'increasing_step': 0.1,  # value that is added to the visibility percentage
    'visibility_maximum': 0.8,   # upper limit
    'visibility_minimum': 0.06,  # lower limit
}
